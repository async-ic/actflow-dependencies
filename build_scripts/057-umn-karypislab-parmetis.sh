...
# GKlib's timers need POSIX.1b. glibc treats _POSIX_C_SOURCE as a floor, but macOS maps
# it straight onto its own feature level, so 199309L there hides strerror_r
# (POSIX.1-2001), which GKlib's error.c calls. Use the level GKlib gives itself on
# linux through target_compile_definitions.
POSIX_LEVEL=199309L
...
-D CMAKE_BUILD_TYPE=Release \
-D OPENMP=set \
-D CMAKE_C_FLAGS="-D_POSIX_C_SOURCE=${POSIX_LEVEL} -Wno-error=incompatible-pointer-types ${CFLAGS}" \
